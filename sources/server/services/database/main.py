...
class BaseModel:
    ...
    def initialize_default_data(self, default_data: list) -> None:
        for record in default_data:
            self.insert(record)

    def remove_extra_fields(self, record: dict) -> dict:
    ...
    def update(self, record_id: int, updates: dict) -> None:
        record = self.get(record_id)
        if not record:
            raise ValueError(f"Enregistrement avec l'id {record_id} non trouvé")
    ...
    def insert_or_get_existing(self, record: dict) -> dict:
        self.validate(record)
    ...
    def get_by(self, key: str, value) -> dict:
